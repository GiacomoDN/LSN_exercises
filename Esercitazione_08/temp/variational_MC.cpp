...
void variational_MC :: initialize(){ // Initialize according to the content of the input files in the ../INPUT/ directory
    ...
    // ### This part has been moved out, in the methods "random_position() and set_current_energy()" ###
        /*
        for(int j=0; j<_ndim; j++){
            _position.setposition(j, _rnd.Rannyu(-1.0,1.0) * _start_range); // uniform distribution in [-_start_range, +_start_range)
        }
...
double variational_MC :: mod_quad_psi_norm(bool xnew){   // Questa probabilmente non mi serve più
    int dim = 0;    // Simulazione 1D
    double x = _position.getposition(dim, xnew); // Position 
    ...
    double norm = 2.0 * sqrt(M_PI) * _sigma * (1.0 + exp(-(_mu * _mu) / sigma2)); 
...
double variational_MC :: Boltzmann(int i, bool xnew){
    double energy_i=0.0;
    double dx, dy, dz, dr;
...
double variational_MC :: get_energy(){
    return _measurement(_index_integral);
}
