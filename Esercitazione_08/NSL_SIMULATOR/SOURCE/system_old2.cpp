...
double System :: Force(int i, int dim){
    ...
    for (int j=0; j<_npart; j++){
         if(i != j){
           ...
           double dr2 = distance(0)*distance(0) + distance(1)*distance(1) + distance(2)*distance(2);
           
           if(dr2 < _r_cut * _r_cut){
                double inv_dr2 = 1.0 / dr2;
                double inv_dr6 = inv_dr2 * inv_dr2 * inv_dr2;
                double inv_dr12 = inv_dr6 * inv_dr6;
                
                // NOTA MATEMATICA: 48/r^14 - 24/r^8 equivale a (48/r^12 - 24/r^6) * (1/r^2)
                f += distance(dim) * (48.0 * inv_dr12 - 24.0 * inv_dr6) * inv_dr2;
           }
         }
    }
    return f;
...
void System :: move(int i){ // Propose a MC move for particle i
    if(_sim_type == 3){ //Gibbs sampler for Ising
         ...
         // De Nardi //
         // Calcolo l'energia E_+ e E_- dello spin, fissati i due vicini. Calcolo la probabilità che sia up. Genero un numero, se è minore faccio spin up se è minore faccio spin down
         
         int s_prec = _particle(pbc(i-1)).getspin();   //Così tratto i casi limite i=0, i=_npart-1
         int s_post = _particle(pbc(i+1)).getspin();
         
         double e_up = -_J*(s_prec + s_post);
         double e_down = -e_up;
         double p_up = exp(- _beta * e_up)/( exp(- _beta * e_up) + exp(- _beta * e_down) );
         if(_rnd.Rannyu() < p_up){
...
double System :: Boltzmann(int i, bool xnew){
    ...
    for (int j=0; j<_npart; j++){
         if(j != i){
           ...
           dr2 = dx*dx + dy*dy + dz*dz; // Niente sqrt()!
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
    ...
    while ( !input.eof() ){
         ...
         if( property == "SIMULATION_TYPE" ){
           ...
           input >> _restart;
         } else if( property == "REVERSE" ){   // Variabile aggiunta, scambia semplicemente conf-1.xyz e config.xyz
           input >> _reverse;
           ...
           for(int i=0; i<_ndim; i++){
                coutf << setw(18) << _side[i];
           }
           coutf << endl;
           coutf << "RHO= " << _rho << endl;  // gli faccio stampare anche rho
         ...
         } else if( property == "INSTANTANEOUS" ){     // Flag aggiunta, mi dice se salvare ogni valore di energia potenziale -> LO USO SOLO PER L'ES. 07.2!!!
           input >> _instantaneous;
    ...
    if(_instantaneous){                                 //Se voglio stampare i dati instantanei di energia potenziale (ES. 07.2)
         // Apri il file quando inizializzi il sistema
         out_epot.open("../OUTPUT/instantaneous_penergy.dat");
    }
...
void System :: initialize_velocities(){
    ...
    if(_restart){
         ifstream cinf;
         if(_reverse){
           cinf.open("../INPUT/CONFIG/config.xyz"); // Se voglio il reverse: inverte le configurazioni di partenza
         }
         else{
           cinf.open("../INPUT/CONFIG/conf-1.xyz"); // Se voglio il tempo normale
         } 
         if(cinf.is_open()){
           string comment;
           string particle;
         ...
         cinf.close();
    } else { // TO BE TEMPORARILY CHANGED IN EXERCISE 4 // Cosa devo cambiare??
        vec vx(_npart), vy(_npart), vz(_npart);
        vx.zeros(); vy.zeros(); vy.zeros();   // Inizializzo a zero
        vec sumv(_ndim);
        sumv.zeros();
        // Inizio modifiche de nardi ////////////////////////////////
        ...
        int b = static_cast<int>(_rnd.Rannyu(0, 3));      //Condizione per la direzione
        int segno = (_rnd.Rannyu() > 0.5) ? 1.0 : -1.0;   //Condizione per estrarre casualmente + o -
        if (b == 0) {
                vx(i) = v_star * double(segno); 
        } 
        else if (b == 1) {
                vy(i) = v_star * double(segno); 
        } 
        else {
                vz(i) = v_star * double(segno);
        }
        
        // Fine modifiche de nardi ////////////////////////
        sumv(0) += vx(i);
        sumv(1) += vy(i);
        sumv(2) += vz(i);
        }
...
void System :: initialize_properties(){ // Initialize data members used for measurement of properties
    ...
    _measure_penergy  = true; //Defining which properties will be measured
    ...
    if (input.is_open()){
         while ( !input.eof() ){
           ...
           if( property == "POTENTIAL_ENERGY" ){
                ...
                index_property++;
                _vtail = 8.0 * M_PI * _rho * ( (1.0 / (9.0 * std::pow(_r_cut, 9))) - (1.0 / (3.0 * std::pow(_r_cut, 3))) ); // TO BE FIXED IN EXERCISE 7
                ...
                index_property++;
                _ptail = 32.0 * M_PI * _rho * _rho * ((1.0 / (9.0 * std::pow(_r_cut, 9))) - (1.0 / (6.0 * std::pow(_r_cut, 3)))); // TO BE FIXED IN EXERCISE 7
           } else if( property == "GOFR" ){
                ofstream coutgr("../OUTPUT/gofr.dat");
                coutgr << "# DISTANCE:     SINGLE_BLOCK_AVE_GOFR:       PROGRESSIVE_AVE_GOFR:        ERROR:" << endl;
                coutgr.close();
                input>>_n_bins;
                _nprop+=_n_bins;
                ...
                coutpv << "# VELOCITY:     SINGLE_BLOCK_AVE_POFV:       PROGRESSIVE_AVE_POFV:        ERROR:" << endl;
                coutpv.close();
                input>>_n_bins_v;
                _nprop += _n_bins_v;
                _bin_size_v = 4.0*_temp/(double)_n_bins_v; // TO BE FIXED IN EXERCISE 4 // Cosa devo fixare??
                _measure_pofv = true;
                _index_pofv = index_property;
                index_property += _n_bins_v;
                ofstream coutf;
                coutf.open("../OUTPUT/output.dat",ios::app);
                coutf << "N_BINS_V= " << _n_bins_v << endl;
                coutf.close();
...
void System :: finalize(){
    ...
    coutf.close();
    if (_instantaneous) { //Chiudi out_epot se lo avevi creato
         out_epot.close();
    }
    return;
}
...
void System :: read_configuration(){
    ifstream cinf;
    if(_reverse){
         cinf.open("../INPUT/CONFIG/conf-1.xyz"); // Se voglio il reverse: inverte le configurazioni di partenza
    }
    else{
         cinf.open("../INPUT/CONFIG/config.xyz"); // Se voglio il tempo normale
    }
    if(cinf.is_open()){
         string comment;
         string particle;
         ...
         cinf >> comment;
         for(int i=0; i<_npart; i++){
           cinf >> particle >> x >> y >> z; // units of coordinates in conf.xyz is sym
           ...
           // Inizio modifiche de nardi /////////////////////////
           //_particle(i).setposition(0, this->pbc(_halfside(0)*x, 0));    // Posizioni DIMEZZATE!
           //_particle(i).setposition(1, this->pbc(_halfside(1)*y, 1));
           //_particle(i).setposition(2, this->pbc(_halfside(2)*z, 2));

           // Fine modifiche de nardi /////////////////////////

           _particle(i).acceptmove(); // _x_old = _x_new   // Perchè lo metto? Per non lasciare vuoto _xold? 
         }
...
void System :: measure(){ // Measure properties
    ...
    int bin; //A cosa serve?
    vec distance;
    distance.resize(_ndim);
    double penergy_temp=0.0, r_ij; // temporary accumulator for potential energy
    ...
    if (_measure_penergy or _measure_pressure or _measure_gofr) {
         for (int i=0; i<_npart-1; i++){
           for (int j=i+1; j<_npart; j++){
                ...
                r_ij = sqrt( dot(distance,distance) );

                // GOFR ... TO BE FIXED IN EXERCISE 7
                if(_measure_gofr){
                  // Trova in quale bin cade la distanza tra la particella i e la j
                  int bin_index_g = static_cast<int>(r_ij / _bin_size);
                  if (bin_index_g >= _n_bins) overflow_count_g++;
                  else 
                  _measurement(_index_gofr + bin_index_g) += 2.0;
                }


                /*
                // NUOVA VERSIONE: Nessuna radice quadrata e niente dot() di Armadillo
                double r_ij2 = distance(0)*distance(0) + distance(1)*distance(1) + distance(2)*distance(2);

                // Confronto il quadrato della distanza con il quadrato del cut-off
                if(r_ij2 < _r_cut * _r_cut){
                  double inv_r_ij2 = 1.0 / r_ij2;
                  double inv_r_ij6 = inv_r_ij2 * inv_r_ij2 * inv_r_ij2; // (1/r^2)^3 = 1/r^6
                  double inv_r_ij12 = inv_r_ij6 * inv_r_ij6;          // (1/r^6)^2 = 1/r^12
                  
                  if(_measure_penergy)  penergy_temp += inv_r_ij12 - inv_r_ij6; 
                  if(_measure_pressure) virial       += inv_r_ij12 - 0.5 * inv_r_ij6;
                }
                */

                //VECCHIA VERSIONE con POW
                if(r_ij < _r_cut){
                  if(_measure_penergy)  penergy_temp += 1.0/pow(r_ij,12) - 1.0/pow(r_ij,6); // POTENTIAL ENERGY
                  if(_measure_pressure) virial       += 1.0/pow(r_ij,12) - 0.5/pow(r_ij,6); // PRESSURE
                }
    ...
    if (_measure_pofv){
         for (int i=0; i<_npart; i++){
           // Modulo della velocità della particella i
           double v_temp = _particle(i).getmodvelocity();
           // Trova in quale bin cade la velocità v
           int bin_index_v = static_cast<int>(v_temp / _bin_size_v);
           if (bin_index_v >= _n_bins_v) overflow_count_v++;
           else 
           _measurement(_index_pofv + bin_index_v) += 1.0;
         }
    }
    // Fine modifiche de nardi //////////////////////////////////////////////////////////////////////////
    ...
    if (_measure_penergy){
         ...
         if(_instantaneous){
           out_epot << penergy_temp << '\n';   //Stampa l'energia istantanea se vuoi
         }
    }
    // KINETIC ENERGY ////////////////////////////////////////////////////////////
    if (_measure_kenergy){
    ...
    // PRESSURE //////////////////////////////////////////////////////////////////
    if (_measure_pressure) _measurement[_index_pressure] = _rho * (2.0/3.0) * kenergy_temp + (48.0 * virial / 3.0) / _volume + _ptail;
    ...
    if (_measure_magnet){
         ...
         _measurement(_index_magnet) = s_i / double(_npart);
    }
    ...
    if (_measure_chi and _measure_magnet){
         _measurement(_index_chi) = _beta * double(_npart) * pow(_measurement(_index_magnet), 2);
...
void System :: averages(int blk){
    ...
    if(_measure_cv){
         _average(_index_cv) = _beta * _beta * double(_npart)* (_average(_index_cv) - pow(_average(_index_tenergy), 2));
    }
    _global_av  += _average;
    ...
    if (_measure_gofr){
        coutf.open("../OUTPUT/gofr.dat",ios::app);
            for (int i=0; i<_n_bins; i++){
                ...
                double bin_center = (i) * _bin_size; // ATT: HO MESSO i * _bin_size, in modo che il primo r = 0! Lo faccio per calcolare correttamente il volume di normalizzazione
                coutf << setw(18) << bin_center
                        << setw(18) << average  // Media del singolo blocco
                        << setw(18) << sum_average/double(blk)
                        << setw(18) << this->error(sum_average, sum_ave2, blk) << endl;
           }   
        coutf.close();
    ...
    if (_measure_pofv){
         /*
         // Se voglio stampare solo l'ultimo
         if (blk == _nblocks){ // Stampa i risultati solo alla fine dell'ultimo blocco
           coutf.open("../OUTPUT/pofv.dat",ios::app); // Apro sovrascrivendo eventuali file vecchi
         ...
         */
         coutf.open("../OUTPUT/pofv.dat",ios::app); // Apro sovrascrivendo eventuali file vecchi
           for (int i=0; i<_n_bins_v; i++){
                // double single_average = _average(_index_pofv + i);
                average  = _average(_index_pofv + i);
                sum_average = _global_av(_index_pofv + i);
                sum_ave2 = _global_av2(_index_pofv + i);
                double bin_center = (i + 0.5) * _bin_size_v;
                coutf << setw(18) << bin_center
                         //<< setw(18) << single_average //Se voglio la media del singolo blocco
                         << setw(18) << average  // Media del singolo blocco
                         << setw(18) << sum_average/double(blk)
                         << setw(18) << this->error(sum_average, sum_ave2, blk) << endl;
           }   
