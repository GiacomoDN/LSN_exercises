...
class CityMap{
    ...
    // Constructor
    CityMap(const Parameters& params); // Costruttor e che prende in input i parametri, il percorso del file di output e il generatore di numeri casuali, e inizializza la mappa delle città (caricamento o generazione delle città e calcolo della matrice delle distanze)

    void generate_cities_file(const std::string& cities_filepath, const std::string& output_filepath, Random& _rnd);  // Funzione che genera il file delle città se non esiste, altrimenti lo lascia invariato
    void load_cities(const std::string& cities_filepath);         // Caricamento delle città da file
