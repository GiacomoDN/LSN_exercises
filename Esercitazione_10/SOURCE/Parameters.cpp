...
void Parameters :: load_parameters(const std::string& filename){
    ...
    while (std::getline(file, line)){
        ...
        if (key == "N_CITIES"){
            ...
            if (!(iss >> migrations)){
                cerr << "ERROR: Invalid value for MIGRATIONS in '" << filename << "'" << endl;
                exit(EXIT_FAILURE);
            }
            has_migrations = true;
        }
    ...
    if (!has_n_cities || !has_map_type || !has_n_individuals || !has_n_generations || !has_mutation_rate || !has_crossover_rate || !has_selection_exponent || !has_n_best || !has_migrations){
...
void Parameters :: save_parameters(const std::string& filepath){
    ...
    file << "MIGRATIONS  " << migrations << endl;
    file << "----------------------" << endl << endl;
